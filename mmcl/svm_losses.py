...
class MMCL_inv(nn.Module):
    def __init__(
        ...
        self.block = torch.zeros(bs, 2 * bs).bool().to(self.device)
        self.block[:bs, :bs] = True
        self.block12 = torch.zeros(bs, 2 * bs).bool().to(self.device)
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        K = compute_kernel(
            F[: nn + 1], F, gamma=self.sigma, type=self.hparams.kernel_type
        )
...
class MMCL_pgd(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        F = torch.cat(torch.unbind(features, dim=1), dim=0)
        K = compute_kernel(
            F[: nn + 1], F, gamma=self.sigma, kernel_type=self.kernel_type
        )
