...
parser.add_argument(
    "--resnet_encoder_ckpt", default="", type=str, help="PATH TO CHECKPOINT"
)
...
print_args(args)
assert args.resnet_encoder_ckpt != "", f"Empty load checkpoint provided: {args.mmcl_checkpoint}"
# Train model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Running on: {device}")
encoder = ResnetEncoder(hparams=args, device=device)
model = ResnetUnsupervisedClassifier(hparams=args, encoder=encoder, device=device)
model.train()
model.save()
model.test()
