...
parser.add_argument("--resnet_supervised_ckpt", default="", type=str, help="Path to the checkpoint to reset the supervised model")

args = parser.parse_args()
print_args(args)
# Train model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = ResnetSupervised(hparams=args, device=device)
model.train()
model.test()
