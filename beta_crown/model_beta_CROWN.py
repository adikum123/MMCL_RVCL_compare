...
def add_single_prop(layers, gt, cls, output_size=10):
    """
    gt: ground truth lablel/real lable
    cls: class we want to verify against
    """
    additional_lin_layer = nn.Linear(output_size, 1, bias=True)
...
def add_single_prop_contrastive(layers, gt, target, output_size=100):
    """
    gt: ground truth vector/real vector
    target: vector we want to verify against
    """
    additional_lin_layer = nn.Linear(output_size, 1, bias=True)
    additional_lin_layer.weight.data = gt - target
    #lin_weights = additional_lin_layer.weight.data
    #lin_weights.fill_(0)
    lin_bias = additional_lin_layer.bias.data
    lin_bias.fill_(0)
    '''
    tem = gt - target
    for kk in range(output_size):
        lin_weights[0, kk] = tem[0][kk].item()
    '''
...
def return_modify_model(model_ori, pred, test, output_size=10, contrastive=False, solve_slope=False, device='cuda', simplify=True):
    layers = list(model_ori.children())
    if simplify:
        if contrastive:
            added_prop_layers = add_single_prop_contrastive(layers, pred, test, output_size)
            modify_layers = added_prop_layers
        else:
            added_prop_layers = add_single_prop(layers, pred, test, output_size)
            modify_layers = added_prop_layers
    else:
        modify_layers = layers
    return copy.deepcopy(nn.Sequential(*modify_layers))


class LiRPAConvNet:

    def __init__(self, model_ori, pred, test, output_size=10, contrastive=False, solve_slope=False, device='cuda', simplify=True, in_size=(1, 3, 32, 32)):
        """
        ...
        self.modify_net = copy.deepcopy(net)
