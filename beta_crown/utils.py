...
def load_cifar_sample_data(normalized=True, MODEL="a_mix"):
    ...
    """
    X = np.load("./beta_crown/data/sample100_unnormalized/"+MODEL+"/X.npy")
    if normalized:
        X = preprocess_cifar(X)
    X = np.transpose(X, (0,3,1,2))
    y = np.load("./beta_crown/data/sample100_unnormalized/"+MODEL+"/y.npy")
    runnerup = np.load("./beta_crown/data/sample100_unnormalized/"+MODEL+"/runnerup.npy")
...
def load_dataset(args):
    ...
    if args.data == 'MNIST':
        ...
        test_data = datasets.MNIST("./beta_crown/data", train=False, download=True, transform=transforms.ToTensor())
        ...
        test_data = datasets.CIFAR10("./beta_crown/data", train=False, download=True, transform=transforms.Compose([transforms.ToTensor(), normalize]))
        #test_data = datasets.CIFAR10("./beta_crown/data", train=False, download=True, transform=transforms.ToTensor())
...
def min_max_value(args):
    ...
    if args.dataset == 'mnist':
        ...
        # set data_max and data_min to be None if no clip
        ans['max'] = torch.reshape((1. - ans['mean']) / ans['std'], (1, -1, 1, 1))
        ans['min'] = torch.reshape((0. - ans['mean']) / ans['std'], (1, -1, 1, 1))
    return ans
...
def load_pickle_results(args):
    ...
    if 'deep' in args.model:
        ...
        gt_results = pd.read_pickle('./beta_crown/data/compare_results/cnn-b-adv.pkl')
        ...
        gt_results = pd.read_pickle('./beta_crown/data/base_100.pkl')
    return gt_results


def model_path(args):
    if args.model == 'cifar_model':
        return "beta_crown/models/cifar_base_kw.pth"
    if args.model == 'cifar_model_wide':
        return "beta_crown/models/cifar_wide_kw.pth"
    if args.model == 'cifar_model_deep':
        return "beta_crown/models/cifar_deep_kw.pth"
    if args.model == 'mnist_cnn_4layer':
        return "beta_crown/models/mnist_cnn_a_adv.model"
    if args.model == 'cnn_4layer_b':
        return "beta_crown/models/cifar_cnn_b_adv.model"
